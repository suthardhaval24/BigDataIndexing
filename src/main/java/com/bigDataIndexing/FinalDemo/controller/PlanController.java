package com.bigDataIndexing.FinalDemo.controller;

/* dhaval created on 2/8/20 inside the package - com.bigDataIndexing.FinalDemo.controller */

import com.bigDataIndexing.FinalDemo.eTag.EtagMap;
import com.bigDataIndexing.FinalDemo.exception.*;
import com.bigDataIndexing.FinalDemo.service.PlanService;
import com.bigDataIndexing.FinalDemo.service.TokenService;
...
//    String a;
    ...
    public ResponseEntity<String> addInsurancePlan(@RequestHeader HttpHeaders headers, @RequestBody String plan) {
        ...
        if (validEtag.size() > 1) {
            return ResponseEntity.status(HttpStatus.CREATED).eTag(etag).body("Plan Saved. PlanId: " + validEtag.get("planid"));
        } else {
            return ResponseEntity.status(HttpStatus.ALREADY_REPORTED).eTag(etag).body("Plan already present");
        }
    }
    ...
    public ResponseEntity<String> deletePlan(@PathVariable(value = "id") String planId, @RequestHeader HttpHeaders headers) {
        ...
        if (planId != null) {
            if ((new PlanService().deleteData(planId)) > 0) {
                ...
                return ResponseEntity.status(HttpStatus.OK).body("Plan is Deleted Successfully!");
            } else
    ...
    public ResponseEntity<String> updatePlan(@PathVariable(value = "id") String planId, @RequestBody String plan, @RequestHeader HttpHeaders header) {
        try {
            ...
            if (validEtag.size() > 1) {
                return ResponseEntity.status(HttpStatus.OK).eTag(etag).body("Plan Saved. Plan id:" + validEtag.get("planid"));
            } else {
                return ResponseEntity.status(HttpStatus.NOT_MODIFIED).eTag(etag).body("Plan is not updated");
            }
        } catch (BadRequest ex) {
            return ResponseEntity.status(ex.getStatus()).eTag(planService.getEtags(planId)).body(ex.getMessage());
        } catch (Forbidden ex) {
    ...
    public ResponseEntity<String> patchPlan1(@PathVariable(value = "id") String planId, @RequestBody String plan, @RequestHeader HttpHeaders headers) throws Exception {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.OK).eTag(etag).body("Plan Saved. Plan id: " + planId);
        } catch (BadRequest ex) {
            return ResponseEntity.status(ex.getStatus()).eTag(planService.getEtags(planId)).body(ex.getMessage());
        } catch (Forbidden ex) {
            return ResponseEntity.status(ex.getStatus()).eTag(planService.getEtags(planId)).body(ex.getMessage());
